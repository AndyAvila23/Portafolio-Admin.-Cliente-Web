...
const AdminsTab = () => {
  ...
  const disabledAdmins = admins.filter(a => a.status === 'inactive').length;
  ...
  const [createForm, setCreateForm] = useState({
    names: '', lastNames: '', idCard: '', phone: '', gender: '', role: 'admin',
  ...
  const processed = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      result = result.filter(a => a.id.toLowerCase().includes(q) || (a.idCard || '').toLowerCase().includes(q) || a.names.toLowerCase().includes(q) || a.lastNames.toLowerCase().includes(q) || a.email.toLowerCase().includes(q));
    }
    result.sort((a, b) => {
      switch (sortBy) {
    ...
    return result;
  }, [admins, searchTerm, filterStatus, filterRole, sortBy]);
  ...
  const handleCreate = (e) => {
    ...
    const prefix = createForm.role === 'admin' ? 'ADM' : 'INS';
    const newAdmin = {
      id: `${prefix}-${Date.now()}`,
    ...
    alert(`${createForm.role === 'admin' ? 'Administrador' : 'Instructor'} creado exitosamente.\n\nCorreo: ${newAdmin.email}\nContraseña temporal: ${tempPassword}\n\nDeberá cambiar su contraseña al primer inicio de sesión.`);
    setIsCreateOpen(false);
    setCreateForm({ names: '', lastNames: '', idCard: '', phone: '', gender: '', role: 'admin', country: 'Ecuador', state: '', city: '', address: '' });
  };
  ...
  const handleSaveProfile = (updatedData) => {
    setUsers(users.map(u => {
      if (u.id === editUser.id) {
        ...
        // Si la cédula cambió, actualizar correo
        if (updatedData.idCard) updated.email = `a${updatedData.idCard}@ulcap.com`;
        return updated;
      }
      return u;
    }));
  ...
  const getRoleLabel = (role) => role === 'admin' ? 'Administrador' : 'Instructor';
    ...
    <>
      ...
      <div className="stats-grid" style={{ marginBottom: '10px' }}>
        <div className="stat-card" style={{ padding: '15px' }}>
          ...
          <div className="stat-details"><h3 style={{ fontSize: '20px' }}>{totalAdmins}</h3><p>Total Administradores</p></div>
        </div>
        <div className="stat-card" style={{ padding: '15px' }}>
          <div className="stat-icon success" style={{ width: '40px', height: '40px', fontSize: '18px' }}><FiCheckCircle /></div>
          <div className="stat-details"><h3 style={{ fontSize: '20px' }}>{activeAdmins}</h3><p>Activos</p></div>
        </div>
        <div className="stat-card" style={{ padding: '15px' }}>
          <div className="stat-icon warning" style={{ width: '40px', height: '40px', fontSize: '18px' }}><FiXCircle /></div>
          <div className="stat-details"><h3 style={{ fontSize: '20px' }}>{disabledAdmins}</h3><p>Deshabilitados</p></div>
        </div>
      </div>
      ...
      <div className="advanced-toolbar">
        ...
        <select value={filterRole} onChange={(e) => setFilterRole(e.target.value)} className="toolbar-select">
        ...
        <button className="auth-button" style={{ margin: 0 }} onClick={() => setIsCreateOpen(true)}>
          <FiPlus style={{ marginRight: '6px' }} /> Agregar Administrador
        </button>
      </div>
                ...
                <tr key={a.id}>
                  <td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: a.role === 'admin' ? 'linear-gradient(135deg, #f59e0b, #ef4444)' : 'linear-gradient(135deg, #0ea5e9, #06b6d4)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '14px', color: '#fff', flexShrink: 0 }}>
                        {a.names.charAt(0)}{a.lastNames.charAt(0)}
                      </div>
                      <div>
                        <div style={{ fontWeight: 600 }}>{a.names} {a.lastNames}</div>
                        <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{a.email}</div>
                      </div>
                    </div>
                  </td>
                  <td style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{a.id}</td>
                  <td>{a.idCard || '—'}</td>
                  <td>{a.phone || '—'}</td>
                  <td>
                    <span className="badge active" style={{ background: a.role === 'admin' ? 'rgba(245,158,11,0.2)' : 'rgba(14,165,233,0.2)', color: a.role === 'admin' ? '#fbbf24' : '#38bdf8' }}>
                      {getRoleLabel(a.role)}
                    </span>
                  </td>
                  <td>{formatDate(a.createdAt)}</td>
                  <td>
                    <span className={`badge ${a.status === 'active' ? 'active' : 'pending'}`}>
                      {a.status === 'active' ? 'Activo' : 'Deshabilitado'}
                    </span>
                  </td>
                  <td>
                    <div className="action-buttons">
                      <button className="action-btn edit" onClick={() => setEditUser(a)} title="Editar"><FiEdit2 /></button>
                      {a.status === 'active' ? (
                        <button className="action-btn delete" onClick={() => setDisableUser(a)} title="Deshabilitar"><FiPower /></button>
                      ) : (
                        <button className="action-btn edit" onClick={() => handleEnable(a.id)} title="Habilitar"><FiPower /></button>
                      )}
                    </div>
                  </td>
                </tr>
              )) : (
                <tr><td colSpan="8" className="no-data">No se encontraron administradores.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {/* MODAL CREAR */}
      <Modal isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} title="Agregar Administrador">
        <form onSubmit={handleCreate} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
          ...
          <div className="form-group">
            <label>Rol</label>
            <select name="role" value={createForm.role} onChange={handleCreateChange} className="toolbar-select" style={{ width: '100%', height: '42px' }}>
          ...
          <h4 style={{ gridColumn: '1 / -1', margin: '10px 0 -5px 0', color: '#818cf8', fontSize: '13px', textTransform: 'uppercase', letterSpacing: '1px' }}>Acceso (No Editable)</h4>
          <div className="form-group">
          ...
          <div style={{ gridColumn: '1 / -1', display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '10px' }}>
            ...
            <button type="submit" className="auth-button" style={{ margin: 0 }}>Crear</button>
          </div>
        </form>
      </Modal>
    </>
